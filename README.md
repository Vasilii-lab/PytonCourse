# Оптимизация параметров поверхностного импеданса

## 📌 Цель проекта
Оптимизация параметров импедансной поверхности для максимального усиления поверхностной волны (Surface Wave, SW) относительно объёмной волны (Volume Wave) на заданной частоте.

## 🧠 Краткое описание физической задачи
Проект решает задачу расчёта и оптимизации трёх параметров (X, A, B), определяющих поверхностный импеданс структуры, с целью максимизации отношения мощности поверхностной волны к мощности объёмной волны. Физически это соответствует проектированию импедансных поверхностей (например, метаповерхностей или искусственных импедансных границ), которые эффективно поддерживают распространение поверхностных волн и подавляют объёмное излучение.

Расчёт мощностей выполняется на основе интегральных выражений для поверхностной и объёмной компонент, а оптимизация проводится в два этапа:
1. Удовлетворение дисперсионного уравнения.
2. Максимизация отношения P_surface / P_volume.

## 📐 Используемые формулы

### 1. **Поверхностный импеданс**
Поверхностный импеданс задаётся в виде:

$$
Z_s(\phi) = jX \cdot \frac{1 + A \sin^2(\phi)}{1 + B \sin^2(\phi)}
$$

где:
- $X$ — реактивное сопротивление (Ом)
- $A$, $B$ — безразмерные параметры формы
- $\phi$ — угол в сферической системе координат

### 2. **Дисперсионное уравнение**
Условие существования поверхностной волны:

$$
\cos^3(\tau) + \frac{jXA}{\eta B} \cos^2(\tau) + \frac{1 - B}{B} \cos(\tau) + \frac{jX(1 - A)}{\eta B} = 0
$$

где:
- $\tau = \pi/2 + 1.3j$ — комплексный угол распространения
- $\eta = 120\pi$ — волновое сопротивление свободного пространства

### 3. **Мощность поверхностной волны**
Интеграл по координате $z$:

$$
P_{\text{surface}} = \int_0^\infty \frac{1}{2} \Re\{E_{z}^{\text{sw}} \cdot (H_{y}^{\text{sw}})^*\} \, dz
$$

где компоненты поля:

$$
E_{z}^{\text{sw}} = \frac{jI_0 k \eta \cos(\tau) \cdot (1 + B \sin^2(\tau))}{D} \cdot e^{-jkz\cos(\tau)} \cdot e^{-j\Omega\sin(\tau)}
$$

$$
H_{y}^{\text{sw}} = \frac{-2\pi j C \cdot \eta \cos(\tau) \cdot (1 + B \sin^2(\tau))}{\sin(\tau) \cdot D} \cdot e^{-jkz\cos(\tau)} \cdot e^{-j\Omega\sin(\tau)}
$$

$$
D = -2jXA\cos(\tau) - 3B\eta\cos^2(\tau) + \eta(1 + B)
$$

где:
- $I_0 = 1.0$ А — амплитуда тока
- $k = 2\pi/\lambda$ — волновое число
- $\Omega = kr$ — безразмерный параметр расстояния
- $C = \dfrac{I_0 k}{2\pi\eta_0}$ — нормировочная константа
- $\eta_0 = 377$ Ом — импеданс свободного пространства

### 4. **Мощность объёмной волны**
Интеграл по углу $\phi$:

$$
P_{\text{volume}} = \int_{-\pi/2}^{\pi/2} \Re\{S_{\text{normal}}\} \, d\phi
$$

где нормальная составляющая вектора Пойнтинга:

$$
S_{\text{normal}} = S_x \sin(\phi) + S_z \cos(\phi)
$$

$$
S_x = -\frac{1}{2} E_z H_y^{*}, \quad S_z = \frac{1}{2} E_x H_y^{*}
$$

Компоненты поля для объёмной волны:

$$
E_z = -\frac{\eta_0}{\sqrt{\Omega}} A_{\text{amp}} \sin(\phi) e^{-j\Omega}
$$

$$
E_x = \frac{A_{\text{amp}}^{e}}{\sqrt{\Omega}} e^{-j\Omega}
$$

$$
H_y = \frac{A_{\text{amp}}}{\sqrt{\Omega}} e^{-j\Omega}
$$

с амплитудами:

$$
A_{\text{amp}} = -\frac{C\sqrt{2\pi}}{e^{-j\pi/4}} \cdot \frac{\cos(\phi)}{(Z_s/\eta_0) + \cos(\phi)}
$$

$$
A_{\text{amp}}^{e} = \frac{C\sqrt{2\pi}}{e^{-j\pi/4}} \cdot \frac{\cos(\phi) \cdot Z_s}{(Z_s/\eta_0) + \cos(\phi)}
$$

### 5. **Целевая функция оптимизации**
Минимизируется функция:

$$
F = -\left[ \log_{10}\left(\frac{P_{\text{surface}}}{P_{\text{volume}}}\right) - P_{\text{surface\_penalty}} - P_{\text{ratio\_penalty}} \right]
$$

где:
- $P_{\text{surface\_penalty}}$ — штраф за малую мощность поверхностной волны ($P_{\text{surface}} < 10^{-9}$ Вт)
- $P_{\text{ratio\_penalty}}$ — штраф за отношение меньше 1 ($P_{\text{surface}}/P_{\text{volume}} < 1$)

## 🚀 Инструкция по запуску
1. Установите необходимые зависимости (см. ниже).
2. Поместите файл `optim_v3.py` в рабочую директорию.
3. Запустите скрипт командой:
   ```bash
   python optim_v3.py
